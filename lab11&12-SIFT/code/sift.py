...
class Sift():
    def __init__(self, img) -> None:
        ...
        self.bins = (self.height + self.width) // 80
    ...
    def get_gradient(self, img):
        ...
        gradient = np.zeros([w, h], "float")
        angle = np.zeros([w, h], "float")
        for i in range(w):
            for j in range(h):
    ...
    def get_feature(self, pos, theta):
        ...
        def vote(x1, x2, y1, y2, xsign, ysign, H, V):
...
tg0 = cv2.resize(tg0, (int(shape[0] * resize_cof), int(shape[1] * resize_cof)))
...
match_img(0.75, w, pad_imgs, sift_tg, sifts)
