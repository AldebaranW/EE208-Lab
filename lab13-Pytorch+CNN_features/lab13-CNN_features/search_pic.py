...
def get_top_scores(image_feature, thresh=0.09, feature_path='./lab13-CNN_features/features'):
...
input_name = './Dataset/8.jpg'
input = default_loader(input_name) # 输入图片，可以更改
...
for name in final_name:
    cv2.imshow('output', cv2.imread('./afhq/val/' + name))
    cv2.waitKey(3000)