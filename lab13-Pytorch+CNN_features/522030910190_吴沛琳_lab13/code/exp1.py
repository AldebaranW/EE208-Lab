...
#     return x**4 + 2 * np.sin(x) + np.cos(x - 1) - 5
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def generate_data(self, num):
        ...
        y_noise = f2(x) + torch.randn([num, 1]) * 3
        return x, y_noise
...
# save model
torch.save(model.state_dict(), 'model' + str(LEARNING_RATE) + '.pth')
...
with torch.no_grad():
    ...
    plt.plot(x, f2(x), color='r', label='ground-truth curve')  # ground-truth curve
    plt.scatter(*(dataset.get_all_data()), label='training set')  # training set
    plt.legend()
    plt.savefig('pic'  + str(LEARNING_RATE) + '.jpg')
    plt.show()
