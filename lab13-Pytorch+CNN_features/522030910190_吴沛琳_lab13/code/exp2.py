...
def test(epoch):
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(state, './checkpoint_resnet50/ckpt_%d_acc_%f.pth' % (epoch, acc))
...
print(test_acc)
test_acc = [str(i) for i in test_acc]

with open('acc_resnet50.txt', 'w') as f:
    f.write(' '.join(test_acc))
